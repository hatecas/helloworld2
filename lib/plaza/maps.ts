 ...
 * 점프와 중력.
 *
 * 최고 높이 = v₀² / (2g) = 620² / 5000 ≈ 77px.
 * (예전엔 g=1500 이라 128px 까지 떠서 발판을 눈감고도 밟았다. 중력을 세게 걸어
 *  최고 높이를 40% 줄였다 — 뜨는 시간도 0.83초 → 0.50초로 짧아져 훨씬 조급해진다)
 *
 * 가로 도달거리는 '같은 높이로 돌아올 때' 로 재면 안 된다. 위 발판에 올라서려면
 * 그만큼 일찍 착지하므로 체공이 짧아진다. dy 만큼 위에 닿는 데 쓸 수 있는 시간은
 *   t = (v₀ + √(v₀² − 2·g·dy)) / g
 * 라서 dy = 62px 면 0.357초, 가로 속도 240 으로 약 86px 뿐이다.
 * 그래서 발판 사이 틈은 60px 이하로 잡는다.
 */
export const FOREST_GRAVITY = 2500;
export const FOREST_JUMP_V0 = 620;
const FOREST_SPEED = 240;

/**
 * 떨어질 때의 중력 — 올라갈 때보다 무겁게.
 *
 * 올라갈 때와 내려올 때가 같으면 정점에서 붕 뜬 것처럼 가볍게 느껴진다.
 * 내려오는 쪽만 1.4배로 무겁게 하면 최고 높이(=닿을 수 있는 높이)는 그대로인데
 * 착지가 훨씬 빨라져서 발이 무거운 느낌이 난다. 옛날 2D 게임들이 쓰던 방법이다.
 *
 * 가로 도달거리는 거의 안 줄어든다 — 정점에서 목표 발판까지는 15px 밖에 안 떨어지므로
 * 내려오는 시간 자체가 짧기 때문이다. (0.109초 → 0.092초)
 */
export const FOREST_FALL_GRAVITY = 3500;
