...
- **↓ = 엎드리기.** 몸통이 46 → 30px 로 낮아져 **머리 위로 지나가는 방해물을 피한다.**
...
- **정상에 오르면 걸린 시간**이 뜨고 같은 숲에 있는 사람들에게도 알려진다(경쟁용).
  시작 발판을 떠나는 순간부터 재고, 시작 발판으로 돌아오면 다시 0 이다.
- **광장 숲 입구 옆 나무 팻말에 상위 3명이 새겨진다** (`forestRecord`).
  ...
  - 기록은 브라우저가 재서 보내므로 마음먹으면 꾸밀 수 있다. 서버가 등반을 따라 계산하는
    ...
    없으므로 구간별 점프 체공 시간의 합(1층 8.3초 / 2층 5.7초)이 하한이고, 그 값을
    시뮬레이션으로 재서 `minClimbMs` 로 뒀다.
