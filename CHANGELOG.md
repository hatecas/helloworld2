...
### OX 퀴즈장 — 광장에서 문으로 들어가는 방

- 광장 위쪽 왼쪽에 **OX 퀴즈장** 문을 세웠다(인내의 숲과 나란히). 광장에 오자마자 퀴즈가
  시작되는 게 아니라, **들어간 사람만** 하는 별도의 맵이다. 채팅은 맵을 가리지 않으므로
  퀴즈장에서 광장 사람들과 계속 얘기할 수 있다.
- 방 안 좌우에 **O · X 칸**이 칠해져 있다. 문제가 뜨면 답이라고 생각하는 칸으로 걸어가
  **정답이 공개될 때 그 칸에 서 있으면 정답**이다. 아무 칸에도 없으면 이번 판은 쉬는 것으로
  본다(연속 정답도 끊기지 않는다). 들어오면 어느 쪽도 아닌 **한가운데**에 서므로,
  가만히 있는 것만으로 답을 고른 셈이 되는 일이 없다.
- 칸마다 **지금 몇 명이 서 있는지**가 뜬다. 남들이 어디로 몰리는지 보는 게 이 놀이의 절반이다.
- 한 판은 **문제 6초 + 정답 공개 3초 = 9초**. 문제 은행은 **53개**(`lib/plaza/quiz.ts`).
- 퀴즈장에서는 **광장의 두 배(380px/s)로 달린다.** 한 문제가 6초뿐이라 광장 속도로는 걸어가는
  데만 시간을 다 쓴다. 이 속도면 한가운데에서 칸까지 0.3~0.7초, 칸에서 반대편 칸까지 1.05초라
  남들이 어디로 몰리는지 보고 막판에 갈아탈 여유가 생긴다.
- **서버도 방장도 없다.** 인내의 숲 방해물과 같은 방식으로, 지금 몇 번째 문제이고 몇 초 남았는지를
  **벽시계(`Date.now`)의 함수**로만 정한다(`quizAt`). 늦게 들어와도 곧바로 같은 문제를 같은 초에
  본다. **채점도 각자 자기 화면에서** 한다 — 정답은 다 같이 알고 있고 남이 어디 서 있는지도
  좌표로 이미 알고 있어서, 신호를 하나도 주고받지 않아도 모두 같은 결과가 나온다.
  (오가는 값은 순위판에 쓰는 누적 점수 둘뿐이고, 그것도 기존 좌표 메시지에 얹었다)
- 그래서 **정답이 클라이언트 파일에 그대로 들어 있다**(개발자 도구로 미리 볼 수 있다).
  서버가 정답을 뒤늦게 내려 주면 막을 수 있지만 그러면 '서버 없이 모두 같은 화면' 이 깨져서,
  미니홈피 놀이인 만큼 열어 두기로 했다.
- **연속 정답**은 이름표 옆에 🔥 로 붙는다. 내 최고 기록은 이 브라우저에만 남는다(서버 기록표 없음).
- **순위판**을 문제판 오른쪽에 작게 뒀다. **이 방에 들어와서 맞힌 수**로 줄을 세우고, 5등까지
  보여 주되 내가 5등 밖이면 내 줄을 따로 붙인다. 방을 나가면 점수는 0으로 돌아가고 목록에서도
  빠진다 — 이 방에 있는 동안의 기록만 세는 판이다.
  - 누적 점수만은 **본인이 세어 좌표(`PosMsg`)에 실어 보낸다.** 채점은 각자 해도 결과가 같지만,
    누적은 각자 자기가 본 것만 세면 **늦게 들어온 사람 화면에는 먼저 와 있던 사람이 전부 0점**으로
    보여서 순위판이 화면마다 달라진다. 새 신호를 만들지 않고 좌표에 두 값을 얹었고, 점수가
    바뀐 순간(가만히 서 있으면 좌표를 안 보내고 있다) 한 번 더 알린다.
- 문제 순서는 **문제 수와 서로소인 걸음으로 건너뛴다** — 연달아 53판(8분)을 봐도 같은 문제가
  다시 나오지 않는다. 다만 벽시계에 매여 있으면 매일 같은 시각에 같은 문제가 나오므로
  **하루에 한 번 시작점을 옮긴다**. 그 지점에서만 겹침 보장이 한 번 끊긴다(확인해 보니
  가장 가까운 재출제가 35판 = 5.3분 뒤라 눈에 띄지 않는다).
- 두 칸 사이는 **400px** 을 비워 뒀다. 갈아타는 데 마음먹을 시간이 들고, 경계에 어정쩡하게
  선 사람이 화면마다 다르게 채점되는 일도 없다.
- 퀴즈장 밖에서는 문제판도 채점도 뜨지 않는다. 정답 공개를 놓친 판은 들어와도
  **뒤늦게 매기지 않는다** — 그때는 다들 자리를 뜬 뒤라 엉뚱한 결과가 나온다.
- 문제판은 **접어 둘 수 있다**(접힌 상태는 기억한다).
- 배경은 광장처럼 **CSS 로만** 그렸다 — 무대 벽·커튼·조명·바닥. 위쪽 150px 은 걸어 들어갈 수
  없게 막았다(문제판이 그 위에 뜨는데, 거기까지 올라가면 미니미가 문제 뒤로 숨는다).
- 맵 기하(칸·문·입장 위치)는 계산해서 확인했다. 입장 200회 전부 **범위 안 · 칸 밖 · 문 밖**에
  섰고, 문 세 개 모두 설 수 있는 자리와 넉넉히 겹치며, 도착 지점은 문 판정 밖이다
  (겹치면 ↑ 를 누르고 있는 동안 두 맵을 왕복한다).
